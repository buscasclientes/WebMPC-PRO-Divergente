...
interface QuickActionsProps {
  ...
  onCaptureSelection: () => Promise<string | null>
  onClear: () => void
}

export default function QuickActions({ onCapturePage, onCaptureSelection, onClear }: QuickActionsProps) {
  ...
  async function handleCapturePage() {
    ...
    try {
      const ctx = await onCapturePage()
      if (ctx) {
        setStatus(`✓ ${ctx.title.slice(0, 15)}…`)
      } else {
  ...
  async function handleCaptureSelection() {
    setStatus('Buscando selección…')
    try {
      const selected = await onCaptureSelection()
      if (selected) {
        setStatus('✓ Selección lista')
      } else {
        setStatus('⚠ Sin selección')
      }
    } catch {
      setStatus('✗ Error')
    } finally {
  ...
  return (
    <div className="flex items-center gap-1 px-2 py-1.5 bg-surface-800 border-b border-surface-700 shrink-0">
      ...
      <button
        ...
        disabled={loading}
        title="Capturar contexto de la pestaña activa (Alt+E)"
      >
        {loading ? (
      ...
      <button
        id="qa-selection"
        className="btn-ghost py-1 px-2 text-[10px]"
        title="Usar texto seleccionado en el Inspector (Alt+S)"
